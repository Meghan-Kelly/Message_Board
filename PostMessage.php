...
<body>
	<?php
		...
		if(isset($_POST["submit"])) {
			...
			// Check that the file both exists AND has at least one message before continuting 
			if((file_exists("messages.txt") === true) && (filesize("messages.txt") > 0)) {
				$MessageArray = file("messages.txt");
				$count = count($MessageArray);
				// Loop through each old messages, and extract only the subject 
				for($i = 0; $i < $count; ++$i) {
			...
			// Check the new subject against the ExisitingSubjects array 
			...
			if($MessageFile === FALSE) {
				echo "<p style='color: red;'>There was an error saving your message!</p>";
			} else {
